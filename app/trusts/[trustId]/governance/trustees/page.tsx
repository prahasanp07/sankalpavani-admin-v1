...
export default function TrusteesRedirect() {
    ...
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(
          ...
          JSON.stringify({ activeTab: 'trustees', parentTab: null })
        );
      } catch (e) {}
    }
    router.replace('/?tab=trustees');
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6 text-primary">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="font-sans text-xs text-on-surface-variant font-medium">
          Loading Trustees in Integrated Workspace...
        </p>
      </div>
    </div>
  );
