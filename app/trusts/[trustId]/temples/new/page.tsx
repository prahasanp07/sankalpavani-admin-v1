...
export default function NewTempleRedirect() {
    ...
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem(
          ...
          JSON.stringify({ activeTab: 'add_temple', parentTab: null })
        );
      } catch (e) {}
    }
    router.replace('/?tab=add_temple');
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6 text-primary">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="font-sans text-xs text-on-surface-variant font-medium">
          Loading New Temple Registration in Integrated Workspace...
        </p>
      </div>
    </div>
  );
